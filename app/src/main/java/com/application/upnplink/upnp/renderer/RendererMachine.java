...
public class RendererMachine implements PropertyChangeListener {
    private UDN udn = new UDN(UUID.randomUUID()); // TODO: Not stable!
    private AndroidUpnpService upnpService;

    public RendererMachine(AndroidUpnpService upnpService) {
        ...
        LocalService<AVTransportService> rendererService = getRendererService();

        // Register the device when this activity binds to the service for the first time
        if (rendererService == null) {
            try {
                LocalDevice rendererDevice = createDevice();
                upnpService.getRegistry().addDevice(rendererDevice);

                rendererService = getRendererService();
        ...
        //System.out.print(rendererService.getManager().getImplementation().getCurrentInstanceIds());

        // Start monitoring the power switch
        rendererService.getManager().getImplementation().getPropertyChangeSupport()
                .addPropertyChangeListener(this);

    }

    protected LocalService<AVTransportService> getRendererService() {
        if (upnpService == null)
            return null;
        ...
        return (LocalService<AVTransportService>)
                binaryLightDevice.findService(new UDAServiceType("AVTransport", 1));
    }
    // DOC:CREATE_DEVICE
    protected LocalDevice createDevice()
        ...
        DeviceDetails details =
                new DeviceDetails(
                        "RendererMachine",
                        new ManufacturerDetails("JP"),
                        new ModelDetails("RendererMachine", "A RendererMachine", "v1")
                );
                ...
                new LastChangeAwareServiceManager<AVTransportService>(service, lastChangeParser) {
                    ...
                    protected AVTransportService createServiceInstance() throws Exception {
                        return new AVTransportService(
                                RendererMachineStateMachine.class,   // All states
                                RendererMachineNoMediaPresent.class  // Initial state
                        );
                    }
                }
        );
